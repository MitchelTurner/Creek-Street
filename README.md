...
- Staff case digest: `GET /api/digest/case/:applicationId/preview` · `POST …/send` · `GET …/last`
- Weekly + outcomes digests deep-link to `/docket/:id` case briefs
- Audited as `digest.case.send`; DRAFT → 404; never includes AI summary body
- Preview/send controls on `/admin/ingest` and `/admin/ops`

**Phase 26 — meeting agenda brief:**

- Public agenda: `GET /api/meetings/:id/agenda` · `…/agenda.pdf` (any mirrored meeting)
- Agenda → public case → structure → `/docket/:id`; outcomes link when `HELD`
- UI at `/meetings/:id`; wired from calendar, search, case briefs, and outcomes
- Sitemap + weekly digest deep-link upcoming meetings; AI summary metadata only

**Phase 27 — meeting summary sheet:**

- Published summary sheet: `GET /api/meetings/:id/summary-sheet` · `…/summary-sheet.pdf` (404 if unpublished)
- Body + per-item text only after human review; deep-links to `/docket/:id`
- UI at `/meetings/:id/summary`; linked from calendar, agenda, and outcomes when published
- Sitemap includes published summary paths only; soft probe `GET /api/meetings/:id/summary` unchanged

**Phase 28 — public decision sheet:**

- Decision sheet: `GET /api/decisions/:id` · `…/sheet.pdf` (404 if missing or linked app not public)
...
- Structure sheet: `GET /api/structures/:slug/sheet` · `…/sheet.pdf` (thin `GET /structures/:slug` kept)
...
- Case digest: `GET /api/digest/case/:applicationId/preview` · staff `POST /api/digest/case/:applicationId/send`
...
- Structure dossier: `GET /api/structures/:slug/sheet` · `GET /api/structures/:slug/sheet.pdf`
